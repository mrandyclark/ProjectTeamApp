...
router.get('/members', function(req, res) { return members(req, res); } );
router.get('/member/:id', function (req, res) { return member(req, res); } );

router.get('/projects', function(req, res) { return projects(req, res); } );
router.get('/project/:id', function (req, res) { return project(req, res); } );
				...
				function(err, allSchedules) {
					...
					_.each(
						...
						function(current, key) {
							...
							var processedDates = rollupDates(groupedSchedules[current._id]);
...
function projects(req, res) {
	...
	async.waterfall([
		...
		function(allProjects, callback) {
			db.members.find({}).sort({name: 1}).exec(
				function(err, allMembers) {
					callback(null, allMembers, allProjects)
				}
			)
		},

		// get all the schedules. since this is a rollup, we'll need 'em all.
		function(allMembers, allProjects, callback) {
			db.schedules.find(
				{},
				function(err, allSchedules) {
					...
					// now process that shit, homie!
					var processedSchedules = {};
					var groupedSchedules = _.groupBy(allSchedules, 'project_id');
	...
	function(err, allDates, allMembers, allProjects, processedSchedules) {
		...
		var inactiveProjects = _.filter(processedSchedules, function(project) { return !project.active; });
		var activeProjects = _.filter(processedSchedules, function(project) { return project.active; });

		res.render(
			'schedule/projects',
			{
				title: "Schedule: Project Rollup",
				editingEnabled: false,
				allDates: allDates,							// used to render columns
				allMembers: allMembers,						// used to render dropdown
				allProjects: allProjects,					// used to render dropdown
				activeProjects: activeProjects,				// the first magic object,
...
function rollupDates(dates) {
	var grouped = _.groupBy(
		dates,
		function(date) { return moment(date.date).format("MM/DD"); } 
	);
	...
	_.each(grouped, function(value, key) {
		var sum = 0;
		_.each(value, function(date) { sum += date.utilization });
		processed[key] = sum / 100;
	});
